  ...
  <script>
    $(function() {
      $("#accordion").accordion();
        ...
        $stmt = $mysqli->prepare("SELECT JokeID, Joke_question, Joke_answer, user_id, username, google_name
        FROM Jokes_table
        ...
        if ($stmt->num_rows > 0) {
          ...
          while ($stmt->fetch()) {
            ...
            echo "<div><p>" . $safe_joke_question  . " -- Submitted by user " . $username . $google_name . "</p></div>";
          }
          echo "</div>";
        } else {
          echo "0 results";
